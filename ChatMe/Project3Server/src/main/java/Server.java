				...
				try {
					...
					while (true) {
						try {
							...
							// if new client joins, notify server and clients online
							if(clientMessage.newUser){
								...
								// Update clients with the received message		//todo: set sender
...
//				}
			...
			public void receiveUserNameResponse() {
				synchronized (listOfClientsID){
					try {
						Message message = (Message) in.readObject();
						System.out.println("Received response from client" + count);

						if(message.success){
							listOfClientsID.add(message.userID);
							this.clientID = message.userID;
							System.out.println("adding " + message.userID);
						}
						else{
							sendUserNames();
							receiveUserNameResponse();
						}
